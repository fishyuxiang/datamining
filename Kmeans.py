        ...
        for j in range(k):
            ...
            for i in range(m):
                ...
                if subCenter[i, 0] == j:  # 计算第j个类别
                    for r in range(m):
                        if subCenter[r, 0] == j:
                            sum_all += distance(data[r, ],data[i, ])
                    if sum_all<minsum:
                        minsum=sum_all
                        centroids[j,]=data[i, ]
...
def showCluser(dataSet,k,centroids,clusterAssment):

    mark=['or','ob','og','ok','^y','+r','sr'];
    if k>len(mark):
